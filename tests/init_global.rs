...
fn run_global_from(cwd: &TempRepo, home: &TempRepo, args: &[&str]) -> Output {
...
fn run_global(home: &TempRepo, args: &[&str]) -> Output {
    run_global_from(home, home, args)
}
...
#[test]
fn global_init_uses_home_when_started_from_a_different_directory() {
    let home = TempRepo::new();
    let cwd = TempRepo::new();

    let output = run_global_from(&cwd, &home, &["init", "-g"]);

    assert!(output.status.success(), "global init should use HOME");
    assert!(home.exists(".claude/settings.json"));
    assert!(home.exists(".claude/rules/standards.md"));
    assert!(home.exists(".codex/hooks.json"));
    assert!(home.exists(".codex/AGENTS.md"));
    assert!(!cwd.exists(".claude"));
    assert!(!cwd.exists(".codex"));
    assert!(!cwd.exists(".lgtm"));
}
