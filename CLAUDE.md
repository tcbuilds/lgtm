...
`.github/workflows/release.yml` builds public x86_64 Linux musl and macOS archives when a `v*` tag is pushed. The tag must exactly match `v` plus the package version in `Cargo.toml`; the current package version is `0.8.0`. The workflow runs tests, packages binaries, publishes SHA-256 files, and creates the GitHub release.
