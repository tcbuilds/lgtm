...
pub(super) fn render_execpolicy(
    ...
    if rules_path.exists() {
        // An unreadable file is preserved rather than replaced: without its
        // contents there is no evidence it was ever generated by lgtm, and
        // clobbering a file the user may have written is the worse error.
        let generated_by_lgtm = match std::fs::read_to_string(rules_path) {
            Ok(existing) => existing.starts_with(GENERATED_HEADER),
            Err(_) => false,
        };
        if !generated_by_lgtm {
            notes.push(format!(
