...
/// First line of every lgtm-generated Codex rules file.
///
/// Doubles as the regeneration marker: a file starting with it was written by
/// lgtm and may be replaced, while anything else is the user's and is preserved.
const GENERATED_HEADER: &str = "# Generated by lgtm from .lgtm/execpolicy.json\n";

/// The Codex `prefix_rule` decision that refuses a command.
///
/// Codex accepts `allow`, `prompt`, and `forbidden`. `deny` is not a member of
/// that set, and Codex rejects the *entire* rules file when it sees one, so an
/// invalid value here silently drops every rule rather than degrading one.
const DENY_DECISION: &str = "forbidden";
...
pub(super) fn render_execpolicy(
    ...
    let existing = read_generated_rules(rules_path, &mut notes);
    if existing.is_none() && rules_path.exists() {
        return Ok((None, notes));
    }
    if config.prohibited_commands.is_empty()
        && config.prohibited_paths.is_empty()
        && existing.is_none()
    {
        return Ok((None, notes));
    }
    let output = render_rules(source_path, &config)?;
    if existing
        .as_deref()
        .is_some_and(|existing| existing.as_bytes() == output.as_slice())
    {
        return Ok((None, notes));
    }
    if existing.is_some() {
        notes.push(format!(
            "regenerating {}; it was generated by lgtm and is replaced rather than preserved",
            rules_path.display()
        ));
    }
    Ok((Some(output), notes))
}

/// Preserve unreadable/user-owned rules; only header-marked files are generated.
fn read_generated_rules(rules_path: &Path, notes: &mut Vec<String>) -> Option<String> {
...
fn render_rules(source_path: &Path, config: &ExecPolicyConfig) -> Result<Vec<u8>, InitError> {
    ...
    for command in &config.prohibited_commands {
        ...
        output.push_str(&format!(
            "prefix_rule(pattern={pattern}, decision=\"{DENY_DECISION}\")\n"
        ));
    }
...
mod tests {
    ...
    /// Codex accepts only `allow`, `prompt`, and `forbidden` as a decision.
    ///
    /// Verified against codex-cli 0.146.0: every other spelling, `deny`
    /// included, fails the Starlark parse with `invalid decision`, and Codex
    /// then drops the *whole* file — so one wrong value disables every rule
    /// while the file still looks installed. Assert the emitted value rather
    /// than that it merely renders.
    #[test]
    fn execpolicy_emits_a_decision_codex_accepts() {
        let root =
            std::env::temp_dir().join(format!("lgtm-execpolicy-decision-{}", std::process::id()));
        std::fs::create_dir_all(&root).expect("temp root");
        let source = root.join(".lgtm/execpolicy.json");
        let contents = r#"{"prohibited_commands":[["rm","-rf"]]}"#;
        let rules = root.join(".codex/rules/lgtm.rules");
        let (rendered, _) =
            render_execpolicy(&source, contents, &rules).expect("execpolicy renders");
        let rendered = String::from_utf8(rendered.expect("rules output")).expect("UTF-8 rules");
        assert!(
            rendered.contains(r#"decision="forbidden""#),
            "expected a Codex-accepted decision, got: {rendered}"
        );
        for rejected in ["deny", "reject", "block", "forbid", "ask"] {
            assert!(
                !rendered.contains(&format!(r#"decision="{rejected}""#)),
                "{rejected} is rejected by the Codex parser"
            );
        }
        let _ = std::fs::remove_dir_all(root);
    }

    /// A rules file lgtm generated is replaced; one the user wrote is not.
    ///
    /// Installations carrying the invalid `deny` spelling must self-heal on the
    /// next init, which preserving them would prevent.
    #[test]
    fn execpolicy_regenerates_its_own_file_but_preserves_a_user_written_one() {
        let root =
            std::env::temp_dir().join(format!("lgtm-execpolicy-regen-{}", std::process::id()));
        let rules = root.join(".codex/rules/lgtm.rules");
        std::fs::create_dir_all(rules.parent().expect("rules parent")).expect("rules directory");
        let source = root.join(".lgtm/execpolicy.json");
        let contents = r#"{"prohibited_commands":[["rm","-rf"]]}"#;
        ...
        .expect("stale generated rules");
        let (rendered, notes) =
            render_execpolicy(&source, contents, &rules).expect("execpolicy renders");
        let rendered =
            String::from_utf8(rendered.expect("stale file is regenerated")).expect("UTF-8 rules");
        assert!(rendered.contains(r#"decision="forbidden""#));
        assert!(notes.iter().any(|note| note.contains("regenerating")));
        ...
        let (rendered, notes) =
            render_execpolicy(&source, contents, &rules).expect("execpolicy renders");
        assert!(rendered.is_none(), "a user-written file must be preserved");
        assert!(
            notes
                .iter()
                .any(|note| note.contains("preserving existing"))
        );
        let _ = std::fs::remove_dir_all(root);
    }
}
